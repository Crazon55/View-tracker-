...
function ProductionTab({ pageFilter, search, readOnly }: { pageFilter: string; search: string; readOnly?: boolean }) {
  ...
  // Reel and Carousel are different pipelines with different stages — the board shows
  // one at a time instead of mixing both sets of columns together. A solo (VE) viewer
  // is locked to whichever pipeline their actual role says — carousel_designer sees
  // Carousel, everyone else (editors/video_editor/ve) sees Reel — read live off the raw
  // role string so this also tracks correctly under admin role-preview, which changes
  // the role without remounting this component. Ops/admin get the free-choice toggle.
  const isCarouselRole = (role || "").split(",").map((r) => r.trim().toLowerCase()).includes("carousel_designer");
  const [contentTypeFilterChoice, setContentTypeFilter] = useState<"reel" | "carousel">("reel");
  const contentTypeFilter = soloView ? (isCarouselRole ? "carousel" : "reel") : contentTypeFilterChoice;
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
        <p style={{ fontSize: 12, color: "var(--pb-dim)", margin: 0 }}>
          {groups.length} idea{groups.length !== 1 ? "s" : ""} in production · one card per idea (grouped across pages)
          {soloView && <span style={{ marginLeft: 8, color: "#a78bfa", fontWeight: 600 }}>· showing only tasks assigned to you</span>}
        </p>
              ...
              {(["reel", "carousel"] as const).map((ct) => (
                <button
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${contentTypeFilter === ct ? "#7c3aed" : "var(--pb-border)"}`,
                    background: contentTypeFilter === ct ? "rgba(124,58,237,0.15)" : "transparent",
                    color: contentTypeFilter === ct ? "#a78bfa" : "var(--pb-dim2)",
                  }}
                >
                  {ct === "reel" ? "Reel" : "Carousel"}
                </button>
              ))}
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {(["stage", "person"] as const).map((v) => (
              <button
                ...
                style={{
                  padding: "5px 12px", borderRadius: 7, fontSize: 11.5, fontWeight: 600, cursor: "pointer",
                  border: `1.5px solid ${viewBy === v ? "#7c3aed" : "var(--pb-border)"}`,
                  background: viewBy === v ? "rgba(124,58,237,0.15)" : "transparent",
                  color: viewBy === v ? "#a78bfa" : "var(--pb-dim2)",
                }}
              >
                {v === "stage" ? "By stage" : "By person"}
              </button>
            ))}
            {viewBy === "person" && (
...
function FrontseatPoolCard({ idea, letter, onDragStart, onClick, onDelete, readOnly }: {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6, paddingRight: 20 }}>
...
// ---------------------------------------------------------------------------
function FrontseatTab({ readOnly, formatFilter = "all", pageFilter = "all", search = "" }: {
  readOnly?: boolean; formatFilter?: string; pageFilter?: string; search?: string;
}) {
  const { pages: allPlaybookPages, pageColors, pageShort, api, id: playbookId } = usePlaybook();
  const playbookPages = pageFilter === "all" ? allPlaybookPages : allPlaybookPages.filter(p => p === pageFilter);
  const qc = useQueryClient();
  const { role } = usePermissions();
  const { user } = useAuth();
  const [detailIdea, setDetailIdea] = useState<any>(null);
  ...
  return (
    ...
    <div style={{ display: "flex", gap: 16, alignItems: "flex-start", minHeight: "calc(100vh - 220px)" }}>
      ...
      {/* ── Left panel: idea pool — a gallery grid (2 cards wide) rather than a
          single-file column, so it stays scannable at 200+ ideas/day, while still
          living on the left next to the page columns. Not relevant once you're only
          seeing your own assigned tasks, so it's hidden for the solo (VE) view. ── */}
      {!soloView && (
      <div style={{
        width: 320, flexShrink: 0, display: "flex", flexDirection: "column",
        background: "var(--pb-panel)", border: "2px solid var(--pb-border)", borderRadius: 16,
        padding: 12, alignSelf: "flex-start",
        boxShadow: "0 0 0 1px rgba(167,139,250,0.18)",
      }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, marginBottom: 6 }}>
          <p style={{ margin: 0, fontSize: 12, fontWeight: 800, color: "var(--pb-ink)", letterSpacing: "-0.01em" }}>
            Ideas Pool
          </p>
          <span style={{ fontSize: 10, color: "var(--pb-faint)", fontWeight: 600, flexShrink: 0 }}>
            {poolIdeas.length} idea{poolIdeas.length !== 1 ? "s" : ""}
          </span>
        </div>
        <p style={{ margin: "0 0 8px", fontSize: 9.5, color: "var(--pb-faint)" }}>
          Drag to assign · new ideas come from Idea Engine
        </p>

        {/* Colour legend — compact pill row */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginBottom: 9 }}>
          {legendStages.map(s => (
            <span key={s} style={{
              display: "inline-flex", alignItems: "center", gap: 4,
              fontSize: 9, fontWeight: 700, color: STATUS_STYLE[s]?.text || "var(--pb-dim)",
              background: STATUS_STYLE[s]?.bg || "var(--pb-chip)", borderRadius: 999, padding: "1.5px 7px",
            }}>
              {STAGE_LABEL[s]}
            </span>
          ))}
        ...
        {/* Pool cards — a 2-up gallery grid instead of one card per row. The panel
            grows to fit every idea rather than scrolling internally, so the whole
            pool is readable in one pass and the page itself does the scrolling. */}
        {isLoading ? (
          <p style={{ color: "var(--pb-faint)", fontSize: 12 }}>Loading…</p>
        ) : poolIdeas.length === 0 ? (
              ...
              return (
                ...
                >
                  ...
                  {assignedPages.length > 0 && (
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 3, marginTop: -3, paddingLeft: 3 }}>
      ...
      <div
        ref={pagesScrollRef}
        style={{ flex: 1, overflowX: "auto", display: "flex", gap: 14, paddingBottom: 20, alignItems: "flex-start" }}
      >
        {playbookPages.map(page => {
          ...
          return (
            <div
              key={page}
              style={{
                minWidth: 225, flex: "1 0 225px", maxWidth: 270,
            ...
            >
              ...
              <div style={{
                ...
                padding: "2px 2px 10px", marginBottom: 6,
                ...
                <span style={{ fontSize: 13, fontWeight: 800, color, letterSpacing: "-0.01em", flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {short}
                </span>
                <span style={{
              ...
              <div style={{
                ...
                {colIdeas.length === 0 && (
                  <div style={{
                    padding: "26px 10px", textAlign: "center", color: "var(--pb-faint)", fontSize: 10.5,
                    border: `1.5px dashed color-mix(in srgb, ${color} 18%, var(--pb-chip))`, borderRadius: 11,
                  }}>
                    Drop an idea here
                  </div>
                )}
    ...
    <div className="fglass-page pb-page" style={{ padding: "16px 20px 40px 64px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 22, flexWrap: "wrap" }}>
        <select value={pageFilter} onChange={e => setPageFilter(e.target.value)} style={sel}>
          ...
          {playbookPages.map(p => <option key={p} value={p}>{pageShort[p] || p}</option>)}
        </select>
        <input
      ...
      <div>
        {tab === "frontseat"     && <FrontseatTab readOnly={!canEditTab("frontseat")} formatFilter={formatFilter} pageFilter={pageFilter} search={search} />}
