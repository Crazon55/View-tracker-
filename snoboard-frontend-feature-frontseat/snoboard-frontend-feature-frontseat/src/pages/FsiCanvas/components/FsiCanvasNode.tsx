...
function FsiCanvasNodeComponent({ data, selected }: NodeProps) {
  ...
  const { fsiNode, canEdit, isFreeform, fieldDefs, onTitleChange, onBodyChange, onPayloadChange } =
    nodeData;
  ...
  return (
    ...
    >
      ...
      {expanded && isFreeform && (
        <div className="border-t border-emerald-900/30 px-3 pb-3">
          <textarea
            rows={4}
            value={body}
      ...
      {expanded && !isFreeform && fieldDefs.length > 0 && (
